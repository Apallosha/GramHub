...
def sponsors_kb():
    ...
    for s in sponsors_db["sponsors"]:
        if s["type"] == "public":
            kb.add(types.InlineKeyboardButton("📢 Канал", url=f"https://t.me/{s['value'][1:]}"))
        elif s["type"] == "private":
            kb.add(types.InlineKeyboardButton("🔒 Приват канал", url=s["value"]))
...
def check_subs(c):
    ...
    if not is_subscribed(c.from_user.id):
        bot.answer_callback_query(c.id, "❌ Ты подписался не на все каналы!", show_alert=True)
        return

    # --- засчитываем реферал ТОЛЬКО после подписки ---
    if u["inviter"] is None:
        start_args = c.message.text.split()
        if len(start_args) > 1:
            inviter_id = start_args[1]
            if inviter_id != str(c.from_user.id) and inviter_id in users_db["users"]:
