...
def sponsors_kb(uid=None):
    kb = types.InlineKeyboardMarkup()
    # Новостник
    kb.add(types.InlineKeyboardButton("📰 Новостник GramHub", url=f"https://t.me/{NEWS_CHANNEL[1:]}"))
    for idx, s in enumerate(sponsors_db["sponsors"]):
        # Добавляем приватные каналы с кнопкой подтверждения
        if s["type"] == "private":
            text = "💎 СПОНСОР (Приват)"
            callback = f"private_{idx}"
            kb.add(types.InlineKeyboardButton(text, url=s["value"], callback_data=callback))
        else:
            kb.add(types.InlineKeyboardButton("💎 СПОНСОР", url=s["value"]))
...
def send_welcome(m):
    ...
    bot.send_message(m.chat.id, text, reply_markup=sponsors_kb(m.from_user.id))
...
def callback_handler(c):
    ...
    # Подтверждение приватного канала
    if c.data.startswith("private_"):
        idx = int(c.data.split("_")[1])
        if idx not in u["private_confirmed"]:
            u["private_confirmed"].append(idx)
            save_json(USERS_FILE, users_db)
            bot.answer_callback_query(c.id, "✅ Приватный канал подтверждён")
        return

    # Проверка подписки
    if c.data == "check_subs":
        # Проверка всех публичных
        if not is_subscribed(c.from_user.id):
            bot.answer_callback_query(c.id, "❌ Ты подписался не на все обязательные каналы!", show_alert=True)
            return
        # Проверка всех приватных подтверждений
        private_indexes = [i for i,s in enumerate(sponsors_db["sponsors"]) if s["type"]=="private"]
        for i in private_indexes:
            if i not in u["private_confirmed"]:
                bot.answer_callback_query(c.id, "❌ Подтвердите все приватные каналы!", show_alert=True)
                return
