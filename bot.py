...
WEBHOOK_URL = "https://gramhub-2qn6.onrender.com" + WEBHOOK_PATH
...
def check_subs(c):
    ...
    bot.send_message(
        c.message.chat.id,
        f"Привет {c.from_user.first_name}! Ты попал в лучшего бота по заработку Gram, "
        "приглашай друзей по реферальной ссылке и зарабатывай Gram!",
        reply_markup=admin_menu() if c.from_user.id in ADMIN_IDS else main_menu()
    )
...
def add_sponsor(m):
    msg = bot.send_message(m.chat.id, "Отправь @канал или invite-link")
    bot.register_next_step_handler(msg, save_sponsor)

def save_sponsor(m):
    sponsors_db["sponsors"].append(m.text.strip())
...
def send_mail(m):
    for uid in users_db["users"]:
        try:
            ...
            pass
    bot.send_message(m.chat.id, "✅ Рассылка завершена", reply_markup=admin_menu())
